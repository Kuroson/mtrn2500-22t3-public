...
auto SharedPointer::operator=(SharedPointer const& other) -> SharedPointer& {
    // Copy then swap
    auto copy = SharedPointer(other);
    std::swap(copy, *this);
    return *this;
}

auto SharedPointer::operator=(SharedPointer&& other) noexcept -> SharedPointer& {
    // Swap
